...
impl ParseWithProtocolVersion for ErrorGetRangeInfo {
    /// Parses an `ErrorGetRangeInfo` event from the given `Scanner` and `protocol_version`.
    ///
    /// The `ErrorGetRangeInfo` event contains the error code and key range of the profiling event.
    ///
    /// # Errors
    ///
