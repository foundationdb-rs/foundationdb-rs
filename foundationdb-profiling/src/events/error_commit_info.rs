...
impl ParseWithProtocolVersion for ErrorCommitInfo {
    /// Parses an `ErrorCommitInfo` event from the given `Scanner` and `protocol_version`.
    ///
    /// The `ErrorCommitInfo` event contains the error code, read conflict range, write conflict range,
    /// mutations, read a snapshot version, report conflicting keys, lock aware, and span ID of the
    /// profiling event.
    ///
    /// The `report_conflicting_keys` field is only present if `protocol_version` is greater than
    /// `ProtocolVersion63`.
    ///
    /// The `lock_aware` field is only present if `protocol_version` is greater than
    /// `ProtocolVersion71`.
    ///
    /// The `span_id` field is only present if `protocol_version` is greater than `ProtocolVersion71`.
    ///
    /// # Errors
    ///
