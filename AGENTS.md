...
- **foundationdb** - the public async API. Key files in `foundationdb/src/`:
  - `api.rs` - `FdbApiBuilder`/`NetworkBuilder`: boot the C API and the network event-loop
    **once per process**; the returned guard must be dropped before exit.
...
- The C API is initialized once per process; never re-boot the network in tests/examples.
