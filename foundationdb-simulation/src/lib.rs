...
pub use bindings::{
    str_from_c, FDBDatabase, FDBWorkload, FDBWorkloadContext, Metric, Metrics, Promise, Severity,
    WorkloadContext,
};
use bindings::{FDBMetrics, FDBPromise, GuestWorkload};
pub use fdb_rt::*;
...
unsafe extern "C" fn workload_setup<W: RustWorkload + 'static>(
    raw_workload: *mut GuestWorkload,
...
unsafe extern "C" fn workload_drop<W: RustWorkload>(raw_workload: *mut GuestWorkload) {
...
macro_rules! register_workload {
    ($name:ident) => {
        ...
        extern "C" fn workloadCFactory(
            ...
            let name = $crate::str_from_c(raw_name);
            let context = $crate::WorkloadContext::new(raw_context);
            $crate::wrap(<$name as $crate::SingleRustWorkload>::new(name, context))
        }
    };
}
