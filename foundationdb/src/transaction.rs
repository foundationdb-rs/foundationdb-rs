...
impl RetryableTransaction {
    ...
    /// database cluster with excessive retries when there is a high level of conflict between
    /// transactions.
    pub async fn on_error(self, err: FdbError) -> Result<RetryableTransaction, FdbError> {
        match Arc::try_unwrap(self.inner) {
            Ok(trx) => match trx.on_error(err).await {
                Ok(trx) => Ok(RetryableTransaction::new(trx)),
                Err(err) => Err(err),
            },
            Err(_) => panic!("closure has kept a reference of the transaction"),
        }
    }
    ...
    /// commit does succeed, the transaction is durably committed to the database and all
    /// subsequently started transactions will observe its effects.
    pub async fn commit(self) -> Result<TransactionCommitted, TransactionCommitError> {
        match Arc::try_unwrap(self.inner) {
